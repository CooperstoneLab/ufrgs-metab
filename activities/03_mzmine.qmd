...
You can actually skip the download process as I put the portable version fo MZmine 3.9.0 in `/fs/ess/PAS2568/MZmine`. But, if you wanted to run MZmine on any supercomputer the process should be the same. Once you are in a virtual desktop, you can open an internet browser and navigate to <https://github.com/mzmine/mzmine3/releases/tag/v3.9.0>. At the time I put this tutorial together, the most recent version of MZmine was 3.9.0. Soon after, version 4.0 was released. We are going to use the older version here. Download the version for the Linux portable version and store it in a known place.

### Open MZmine

Once you have downloaded MZmine, unzip the file. If you are using the already downloaded verison, click on the right most folder at the bottom of your screen, and insert `/fs/ess/PAS2568/MZmine` into the path.  and then right click on the `bin` directory and select `Open a terminal here`.
...
Create a directory (i.e., folder) with your name (no spaces) within `/fs/ess/PAS2568`. This will allow each of you to store your files separately from your classmates. For example, I would make a folder called `jess` within `/fs/ess/PAS2568`. This would mean then I am saving my files in `/fs/ess/PAS2568/jess`.
...
2. click on `Files`
3. select `/fs/ess/PAS2568/your-folder-name` and download any files so you can have the locally on your computer.
